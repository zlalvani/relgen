...
import { omit, parallel, unique } from 'radashi';
...
const MetadataSchema = z.object({
  title: z.string(),
  complexity: z.enum(['trivial', 'minor', 'major']),
});

function getCommentMetadata(body?: string) {
  if (!body) {
    return null;
  }
        ...
        ascribe: async (
          ...
          const metadata = await parallel(
            ...
            async (pr) => {
              ...
              return (
                getCommentMetadata(relgenComment) ??
                (async () => {
                  ...
                  return omit(result.object, ['description']);
                })()
              );
            }
          );
